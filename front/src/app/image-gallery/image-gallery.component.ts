...
export class ImageGalleryComponent implements OnInit {
  ...
  updateImageGallery(data: any) {
    ...
    for(let i = 0; i < paths.length; i++) {
      this.allImages.push({src: `https://storage.googleapis.com/trabalho_final/dataset/images_SIDEWALK/${paths[i]}`,
                       thumb: `https://storage.googleapis.com/trabalho_final/dataset/thumbnails_SIDEWALK/${paths[i]}`, 
                       id: ids[i], 
                       index: i, 
                       width: 93, 
                       height: 93, 
  ...
  onAfterSlide = (detail: any): void => {
    ...
    const string = image.src.replace("https://storage.googleapis.com/trabalho_final/dataset/thumbnails_SIDEWALK/","")
