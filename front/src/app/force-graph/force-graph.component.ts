...
export class ForceGraphComponent implements OnInit {
  ...
  buildTooltip(node: any, queryType: number) {
    ...
    else if(queryType == 1 || queryType == 4) for(let i = 0; i < node.imagesQuery.length; i++) str += `<img style="max-width: 64px; max-height: 64px; margin-left: 12px" src="${node.imagesQuery[i].replace('dataset', 'https://storage.googleapis.com/pm2023/art')}"></img>`;
    else {
      for(let i = 0; i < node.textsQuery.length; i++) str += `${node.textsQuery[i]}<br>`;
      for(let i = 0; i < node.imagesQuery.length; i++) str += `<img style="max-width: 64px; max-height: 64px; margin-left: 12px" src="${node.imagesQuery[i].replace('dataset', 'https://storage.googleapis.com/pm2023/art')}"></img>`;
    }
    return str
  }
